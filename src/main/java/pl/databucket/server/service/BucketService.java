...
public class BucketService {
    ...
    public Bucket createBucket(BucketDto bucketDto) throws ItemAlreadyExistsException, ItemNotFoundException {
        ...
        if (bucketDto.getGroupsIds() != null && !bucketDto.getGroupsIds().isEmpty()) {
        ...
        if (bucketDto.getUsersIds() != null && !bucketDto.getUsersIds().isEmpty()) {
            List<User> users = userRepository.findAllByIdIn(bucketDto.getUsersIds());
            bucket.setUsers(new HashSet<>(users));
        }

        if (bucketDto.getTeamsIds() != null && !bucketDto.getTeamsIds().isEmpty()) {
    ...
    public Bucket modifyBucket(BucketDto bucketDto)
        throws ItemAlreadyExistsException, ItemNotFoundException, ModifyByNullEntityIdException {
        ...
        if (!(currentGroupsIds.containsAll(newGroupsIds) && newGroupsIds.size() == currentGroupsIds.size())) {
            Set<Long> toAdd = newGroupsIds.stream().filter(element -> !currentGroupsIds.contains(element))
            ...
            Set<Long> toRemove = currentGroupsIds.stream().filter(element -> !newGroupsIds.contains(element))
                .collect(Collectors.toSet());
            if (toRemove.size() > 0) {
    ...
    public List<Bucket> getAccessTreeBuckets(User user) {
        return bucketRepository.findAllByDeletedOrderById(false).stream()
            .filter(bucket -> hasUserAccessToBucket(bucket, user)).toList();
    }

    public boolean hasUserAccessToBucket(Bucket bucket, User user) {
        boolean accessForUser = !bucket.getUsers().isEmpty() && bucket.getUsers().contains(user);

        if (accessForUser) {
            return true;
        } else {
            boolean accessByRole =
                bucket.getRole() != null ? user.getRoles().contains(bucket.getRole()) : !bucket.getTeams().isEmpty();
            boolean accessByTeam =
                !bucket.getTeams().isEmpty() ? !Collections.disjoint(bucket.getTeams(), user.getTeams())
                    : bucket.getRole() != null;
